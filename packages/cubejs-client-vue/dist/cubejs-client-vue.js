  ...
  methods: {
    copyQueryFromProps: function copyQueryFromProps() {
      ...
      this.timeDimensions = (timeDimensions || []).map(function (m, i) {
        return _objectSpread$1({}, m, {
          dimension: _objectSpread$1({}, _this4.meta.resolveMember(m.dimension, 'dimensions'), {
    ...
    addMember: function addMember(element, member) {
      ...
      if (element === 'timeDimensions') {
        ...
        if (mem) {
          var dimension = _objectSpread$1({}, this.meta.resolveMember(mem.name, 'dimensions'), {
            granularities: this.granularities
          });

          mem = _objectSpread$1({}, mem, {
            granularity: member.granularity,
            dateRange: member.dateRange,
            dimension: dimension,
    ...
    setMembers: function setMembers(element, members) {
      ...
      members.forEach(function (m) {
        if (element === 'timeDimensions') {
          ...
          if (mem) {
            var dimension = _objectSpread$1({}, _this5.meta.resolveMember(mem.name, 'dimensions'), {
